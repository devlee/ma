...
| 补充 M:文档主题升维与四层重组 | 替换原文标题与目录结构 | 内容已超出"人群、场景、品类"的主题范围 |
...
原标题《MA 的增长:人群、场景、品类》只覆盖**定位层**;文档现有内容中,引擎层(飞轮、信任工程、custom size)、纪律层(闸门、保护)、期权层(中台、第二曲线)的篇幅已超过定位层。标题与内容不匹配,团队会以为这是一份"选品类"的文档,而错过它真正的重心。

### 建议的新主题与四层结构

**新标题:《MA 增长战略:定位 · 引擎 · 纪律 · 期权》**(原"人群、场景、品类"降为第一部分的副标题保留)

| 层 | 回答的问题 | 收纳的内容 |
|---|---|---|
| **第一部分 · 定位:增长从哪来**(人群、场景、品类) | 打哪块地 | 原文 1 核心结论、2 occasion 划分与资产、市场 A/B 分析 + 补充 A(四象限)、B(市场对照)、C(同心圆) |
| **第二部分 · 引擎:增长怎么来** | 靠什么复利 | 原文 3 AZ 启发(3.1.0 套利 vs 品牌 [J]、3.1.1–3.1.7 飞轮、3.1.8 信任工程 [K]、3.1.9 custom size [L])、原文 4 mewmews 启发(第二张价目表) |
| **第三部分 · 纪律:增长怎么不翻车** | 怎么验证、护什么 | 补充 E(验证、闸门、杀死条件)、F(三轨制、信任分层、ready-to-ship、组织)、H(数据置信度) |
| **第四部分 · 路线:时间表与期权** | 先做什么、后做什么 | 补充 G(三层行动时间表)、D(中台定义)、期权层(第二品牌、LWD、收购) |

### 讲解时的一句话导览(建议开场使用)

> "这份文档回答四个问题:增长从哪来(定位)、怎么来(两台引擎:让订单自我繁殖的飞轮,和让用户敢买的信任工程)、怎么保证不翻车(闸门和保护纪律)、以及走通之后下一步是什么(中台和第二品牌期权)。"
...
### 第一层:现在就做(90 天内,不等任何闸门)——按五条工作流编组

> 编组原则:每条工作流可指定一个负责人;流内按依赖排序,流间可并行。
> (v2 更新:纳入信任工程四件套 [3.1.8]、custom size 激活六动作与无理由退试点 [3.1.9])

**工作流 A · 数据与验证(所有动作的开关,最先启动)**

| # | 动作 | 说明 | 成本 |
|---|---|---|---|
| A1 | 拉 evening 买家数据 | 5.1 开关性验证:年龄结构 / custom size 使用率 / 与妈妈装重合度,**本周** | ≈0 |
...
**工作流 C · 信任工程与 custom size(本次更新新增最多)**

| # | 动作 | 说明 | 成本 |
|---|---|---|---|
| C1 | 信任线圈定 | 选 200–500 款;金样对照片验收;色卡改从大货批次剪 | 中 |
| C2 | 发货前实拍 | 过 QC 时拍两张实物照发用户,成本≈0,先行上线 | ≈0 |
| C3 | 先下单后量体 + 承诺徽章 | 结账流程改造 + PDP 尺码选择器旁"完美合身承诺"(3.1.9 两大转化杠杆) | 低 |
| C4 | 三级承诺 SOP | 改衣补贴→免费重做→积分兜底的客服流程与财务口径(依赖 C3 同步) | 低 |
| C5 | 三问推断粗版 + 软尺随箱 | 常穿码+身高+体重推断全套尺寸并发用户确认;软尺随色卡和每个包裹寄出 | 低 |
| C6 | Event Guarantee 上线 | 婚礼前 X 天出问题加急重做或全退(依赖 B1) | 低 |
| C7 | 定制单无理由退试点启动 | 按 3.1.9.4 四道围栏圈定,PDP 五五分流(依赖 C3/C4;读数跨层,3–4 个月) | 学费封顶≈$9K |
...
| # | 动作 | 说明 | 成本 |
|---|---|---|---|
| E1 | LWD / bachelorette 品类测试 | 30–80 款站内 collection,锁死在 Bride 语境,季前上线 | 中 |
| E2 | ready-to-ship 子线立项 | 关键路径,周期最长,先做方案与首批 SKU 圈定(按中台"可复用"标准建) | 中–高 |
...
- **无理由退扩围**:试点过 3.1.9.4 决策规则后,扩大到全部信任线,再评估全站;
...
### 3.1.9.4 更进一步:定制单无理由退的小范围试点(承诺梯子的"第四级")

> 全行业(Azazie、JJ's House)的定制单都是 final sale。如果试点跑通,missacc 拿到一句独有的话:
> **"唯一一家定制尺码也支持无理由退的礼服品牌"**——同时打在 JJ's(25% restocking fee)和 Azazie(定制 final sale)最疼的位置,且两者因各自结构(全品类退货敞口 / 多件订单敞口)极难跟进。

**结构性优势:妈妈装品类天然抗 wardrobing。** 她提前 3–6 个月下单,交付后距婚礼通常还有 1–3 个月;退货窗口设"签收后 14 天",窗口在婚礼前早已关闭——穿完再退在结构上不可能。再用 event date 字段加一道保险:**只对"交付日早于婚礼 30 天以上"的订单开放**,临期加急单排除。这个防御是交付即穿的 party dress 玩家学不了的。

**算账框架:**

```
不可退:   期望贡献 = C                    (C = 客单 − COGS − 运费 − 支付费)
无理由退: 期望贡献 = (1−r) × C − r × L
          r = 可退政策下的定制退货率(未知数①)
          L = 单次退货净损失 = COGS + 双程运费 + 处理 − 残值(未知数②)
```

示意测算(假设值,测试目的就是替换它们):C ≈ $100,L ≈ $79,r = 10% → 期望贡献 $82,比不可退少 $18。**但正确对照是"没有政策时用户去了哪":**
...
**盈亏平衡:(新增转化 × $82)+(标码→定制迁移 × $7)>(存量定制 × $18)。** 三个未知数(r、残值率、转化提升)全部由测试实测。

**残值单独经营,可大幅左移平衡点**:定制尺寸中相当比例在标准码 ±1 码内,退回品走 outlet/sample sale 频道 4–6 折清货,L 可从 $79 压到 $50 以下;退款提供"现金 or 110% 店铺积分"双选项,进一步减少现金流出。

**测试设计(四道围栏锁死最坏情况):**
1. 只对信任线 SKU;2. 只对交付日早于 event date 30 天以上的订单;3. 先只开美国市场(本地退货集散);4. 总量闸门(如前 500 单)——按最坏假设每单拖累 $18,学费上限 ≈ $9,000。

实验结构:PDP 五五分流,A 组挂"14 天无理由退"徽章,B 组挂"完美合身承诺"(一次测试同时回答"承诺够不够、还是必须可退")。

**读数节奏(防误判)**:领先指标(定制占比、CVR)2–4 周可读;滞后指标(退货率、残值回收)需等生产+交付+14 天窗口走完,**约 3–4 个月才完整**——中途既不宣布胜利也不宣布失败。

**决策规则(事先写死):**
- 转化无显著提升 → 政策不解决真问题,回退"完美合身承诺"(便宜一档);
- 转化提升且 r ≤ 实测盈亏平衡阈值 → 扩大到全部信任线,再评估全站;
- 转化提升但 r 超阈值 → 降一档再测:积分全退+现金退收小额费,或窗口收紧到 7 天。

### 3.1.9.5 90 天落地与指标

顺序:① 先下单后量体流程 + PDP 承诺徽章(转化杠杆,先上)→ ② 三级承诺的客服 SOP 与财务口径 → ③ 三问推断粗版 → ④ 软尺随箱 → ⑤ AI 量体供应商评估 → ⑥ 视频量体试点(仅妈妈装)→ ⑦ 定制单无理由退试点(3.1.9.4,依赖①②就位后启动)。
