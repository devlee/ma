...
def summ(g):
    ...
    return pd.Series({
        ...
        '颜色图覆盖率_非婚纱': nb['有颜色图'].mean() if len(nb) else float('nan'),
        '素材完备率': g['素材完备'].mean(),
        '完备款数': g['素材完备'].sum(),
    })
...
L3 = e[(e['评论OK']) & (~e['颜色图OK'])]
L4 = e[(e['细分等级'] == 'E2') & (e['Reviews数'] >= 30) & (e['颜色图OK']) & (e['买家秀OK'])]
...
cols = ['SPU', '子品类', '是婚纱', '细分等级', '销量件数', 'GMV_产品表', '产品单价', '上架天数',
        '买家秀数量', 'Reviews数', '有颜色图', '素材缺项', '产品状态']

def prep(df):
    d = df[cols].copy()
    d['有颜色图'] = d.apply(lambda r: '不适用(婚纱)' if r['是婚纱'] else ('有' if r['有颜色图'] else '无'), axis=1)
