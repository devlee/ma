...
facts = [
    ...
    ("高客单高毛利", "婚戒是强情感、强定制品类，服务与设计溢价空间大"),
]
y = Inches(2.32)
for t, d in facts:
...
add_text(s, rx + Inches(0.4), Inches(2.0), Inches(4.8), Inches(0.5),
         [("我们卖什么", 19, True, GOLD)])
prods = [
    ("核心产品", "培育钻婚戒 / 订婚戒（IGI·GIA 证书裸钻 + 18K 金/铂金戒托）"),
    ("核心服务", "1v1 定制顾问全程陪跑：选钻 → AI 出图 → 大师手作 → 保险直邮"),
    ("交付模式", "一件一做、按单生产，支持刻字、改圈、终身保养"),
    ("销售渠道", "品牌独立站 DTC 直达消费者，不依赖平台与门店"),
]
y = Inches(2.62)
for t, d in prods:
...
add_text(s, Inches(0.62), Inches(3.42), Inches(12.1), Inches(0.55),
         [[("＝  ", 24, True, GOLD), ("克拉先生 Mr. Karat：一件一做的美国培育钻婚戒定制品牌", 21, True, NAVY)]],
         align=PP_ALIGN.CENTER)
...
lights = [
    ...
    ("定制服务承接", "1v1 顾问承接高客单决策，转化与复购的信任基石", RED),
]
...
funnel = [
    ...
    ("转化", "1v1 定制顾问：选钻教学 → 3D 效果图 → 分期支付", Inches(3.8)),
...
frows = [
    ...
    ("广告及营销投放", "150", "380", "820"),
    ("货品及履约成本", "290", "830", "1,950"),
    ("团队及运营费用", "85", "150", "250"),
    ("净利润", "▲ -25（战略亏损）", "140（利润率 9%）", "580（利润率 16%）"),
]
row_h = Inches(0.62)
...
notes = [
    ...
    ("利润率锚点", "第 3 年稳态利润率 15%~20%，利润来自设计与定制服务溢价，而非裸钻差价"),
    ("长期愿景", "跑通单站模型后复制放大，3~5 年冲击年营收 5,000 万美金量级"),
]
ny = Inches(5.75)
for t, d in notes:
...
add_text(s, Inches(1.05), Inches(2.7), Inches(5.25), Inches(2.2),
         [("· 一件一做，零库存资金占用", 14, False, GREY),
          ...
          ("· 利润锚定设计与服务溢价，15%~20%", 14, False, GREY)],
         space_after=10)
