...
rules = [
    ...
    ("单个作图投放计划", "\"单个作图投放计划\"表给出平滑后的每周排产明细: 在\"每款的图须在其上架周前一周的周五完成、SPU生成后才进作图队列、春节两周不排工\"的约束下, 按最早截止优先排产, 得到恒定25款/周的最小可行投放量(2/1周13款收尾)。注意: 甘特总览与每周工作清单中的\"作图\"窗口是最晚完成基线, 单个作图的实际开工与排量以本投放计划表为准(SPU完成即可提前开工)。25是数学下限、无缓冲, 建议实际投放维持当前30款/周: 按25的计划表执行, 每周富余约5款用于吸收延误或在架款优化; 2月起的空档周可提前投放3月批次。"),
...
for month in MONTHS:
    for cat in CATS:
        for b in [x for x in BATCHES if x["month"] == month and x["cat"] == cat]:
            qty = b["qty"]
            no_sel = not b["sel"]  # 选款已完成或无需选款的批次不再计选款人力
            name = CAT_NAMES[cat] + (f"({b['sub']})" if b["sub"] else "")
...
for w in WEEKS:
    if w in HOLIDAY_WEEKS:
        ...
        for j in range(1, len(cols) + 1):
            ...
            style_cell(c2, LEFT if j in (1, 2, 12) else CENTER)
            c2.fill = STAGE_FILL["假期"]
        r += 1
        continue
...
for w in WEEKS:
    ...
    single, batch, tags = art_load(w)
    smoothed = smoothed_load(w)
    if tags or smoothed > 1e-9:
        mark = " <== 平滑后仍超当前投放" if smoothed > SINGLE_CURRENT else ""
        print(f"  {w}: 单个基线 {single:5.1f} -> 平滑 {smoothed:5.1f}"
              f" (理论{smoothed/SINGLE_CAP:4.0%}/当前投放{smoothed/SINGLE_CURRENT:4.0%})"
